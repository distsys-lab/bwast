...
public abstract class ModifiedDefaultSingleRecoverable implements Recoverable, SingleExecutable {
    ...
    public ModifiedDefaultSingleRecoverable() {
    ...
    private byte[] executeOrdered(byte[] command, MessageContext msgCtx, boolean noop) {
        ...
        if (msgCtx.isLastInBatch()) {
            // Create checkpoint at cid == 0
            if ((cid % checkpointPeriod) == 0) {
    ...
    public StateManager getStateManager(TOMConfiguration config) {
        if (stateManager == null) {
            switch (config.getTransferAlgorithm()) {
                case "sort":
